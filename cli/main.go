...
func buildAuditPrompt(repoRoot string) (string, error) {
	...
	sb.WriteString("Which SOLID/DRY principle is violated and why, referencing exact code.\n\n")
	sb.WriteString("```diff\n- old code\n+ fixed code\n```\n\n")
	sb.WriteString("**AI agent fix prompt:** One-paragraph instruction a coding agent can execute directly to fix this.\n\n")
	sb.WriteString("If there are no violations: write `_No issues found._` and skip subsections.\n\n")
